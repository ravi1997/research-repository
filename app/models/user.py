...
class OTP(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    client_id = db.Column(db.Integer, ForeignKey('clients.id'), nullable=False)  # Corrected table name
    otp = db.Column(db.String(7), nullable=False)
    created_at = db.Column(DateTime, server_default=func.now(), nullable=False)
    status = db.Column(SQLAlchemyEnum(ValidState), nullable=False, server_default=f'{ValidState.VALID}')
