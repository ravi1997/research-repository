...
class Client(db.Model):
    ...
    status = db.Column(SQLAlchemyEnum(ValidState), index=True, nullable=False, server_default=f"'{ValidState.VALID.value}'")
    ...
    def __repr__(self):
        return (f"<Client(id={self.id}, client_session_id='{self.client_session_id}', "
                f"user_id={self.user_id}, status='{self.status.value}', ip='{self.ip}')>")

class OTP(db.Model):
    __tablename__ = "otps"  # Updated table name to be plural
    id = db.Column(db.Integer, primary_key=True)
    client_id = db.Column(db.Integer, ForeignKey('clients.id'), index=True, unique=True, nullable=False)
...
class User(db.Model):
    ...
    status = db.Column(SQLAlchemyEnum(UserState), index=True, nullable=False, server_default=f"'{UserState.CREATED.value}'")
    ...
    def __init__(self, firstname, mobile, email, employee_id, department, designation, date_expiry,
                 middlename=None, lastname=None, status=UserState.CREATED, updated_at=None, roles=None):
    ...
    def __repr__(self):
        return (f"<User(id={self.id}, name='{self.firstname} {self.middlename or ''} {self.lastname or ''}', "
                ...
                f"status='{self.status.value}', role='{[role.role.value for role in self.roles]}')>")
...
class UserRoles(db.Model):
    ...
    def __repr__(self):
        return f"<UserRole(id={self.id}, user_id={self.user_id}, role='{self.role.value}')>"
