...
def verify_roles(f,roles):
	...
	def decorated_function(*args, **kwargs):
		...
		if any(session.user.has_role(role) for role in roles):
			return f(session,*args, **kwargs)
		else:
			error_logger.error(f'User is not authorised')
			return jsonify({"message":"Unauthorized User"}),401
...
def verify_create_roles(f):	
	return verify_roles(f,[UserRole.FACULTY,UserRole.SCIENTIST])
	
def verify_delete_roles(f):
	return verify_roles(f,[UserRole.LIBRARYMANAGER])

def verify_edit_roles(f):
	return verify_roles(f,[UserRole.FACULTY,UserRole.SCIENTIST,UserRole.LIBRARYMANAGER])

def verify_duplicate_roles(f):
	return verify_roles(f,[UserRole.LIBRARYMANAGER])

def verify_ownership_roles(f):
	return verify_roles(f,[UserRole.FACULTY,UserRole.SCIENTIST,UserRole.RESIDENT])
