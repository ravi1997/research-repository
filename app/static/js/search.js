...
function createPagination(containerId, currentPage, totalPages) {
    ...
    function createButton(label, targetPage, isDisabled) {
        ...
        button.className = isDisabled
            ? "bg-blue-800 text-white font-bold py-2 px-2 text-sm xl:px-4 xl:text-md rounded opacity-50 cursor-not-allowed"
...
function renderArticle(article) {
    ...
    divContent.classList.add(
        ...
        "duration-100", "ease-in", "hover:bg-teal-900", "hover:text-teal-100",
    ...
    article.authors.forEach((author, index) => {
        if (author.employee_id) {
            ...
            authorLink.href = `/author?q=${author.employee_id}`;
            authorLink.target = "_blank";
            authorLink.innerHTML = `<span class="mb-1 text-l font-bold">${author.fullName}</span>`;
            divText.appendChild(authorLink);
        } else {
            divText.appendChild(document.createTextNode(author.fullName));
        }
        if (index < article.authors.length - 1) {
            divText.appendChild(document.createTextNode("; "));
        }
    });
    ...
    // View Icon
    let divViewIcon = document.createElement("div");
    divViewIcon.classList.add("flex", "flex-col", "gap-2", "p-2", "align-middle");
    ...
    divViewIcon.appendChild(viewLink);

    let divEditIcon = document.createElement("div");
    divEditIcon.classList.add("flex", "flex-col", "gap-2", "p-2", "align-middle");

    let editLink = document.createElement("a");
    editLink.href = `/article/edit/${article.uuid}`;
    editLink.target = "_blank";
    editLink.ariaLabel="Edit Article : " + article.title; 
    editLink.innerHTML = `
    <svg 
      width="24px" 
      height="24px" 
    ...
    >
      ...
      />
                        ...
                        `;
    divEditIcon.appendChild(editLink);


    divContent.appendChild(divViewIcon);
    console.log(roles);
    if (['LIBRARYMANAGER', 'FACULTY', 'SCIENTIST'].some(role => roles.includes(role))) {
        divContent.appendChild(divEditIcon);
    }
